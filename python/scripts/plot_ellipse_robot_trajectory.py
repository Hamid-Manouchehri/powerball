"""
Plot the ellipse reference and the logged robot XY trajectory.
...
Outputs:
    Figure:
        1) ellipse centerline [m]
        2) ellipse left and right boundaries [m]
...
read_csv_file = SCRIPT_DIR / "data/ellipse_kang_indirect_var_adm_schunk.csv"  # TODO
...
semi_axis_x = 0.20  # TODO ellipse semi-axis along x [m]
semi_axis_y = 0.10  # TODO ellipse semi-axis along y [m]
band_width = 0.045  # TODO corridor thickness [m]
ellipse_rotation_deg = 0.0  # TODO ellipse rotation [deg]
...
def generate_ellipse_xy():
    """
    Generate the ellipse centerline.
    ...
    theta = np.linspace(0.0, 2.0 * np.pi, num_reference_points,
                        endpoint=False)
    rot = np.deg2rad(ellipse_rotation_deg)
...
def generate_ellipse_boundaries(x_ref, y_ref):
    """
    Generate the left and right boundaries of the ellipse band.
