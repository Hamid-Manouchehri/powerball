"""
Plot the four-leaf clover reference and the logged robot XY trajectory.
...
Outputs:
    Figure:
        1) clover centerline [m]
        2) clover left and right boundaries [m]
...
read_csv_file = "/home/hamid-tuf/projects/powerball/matlab/VAC/raw_data/test_four_leaves_chen_kf_var_adm_schunk.csv"  # TODO
# read_csv_file = "/home/hamid-tuf/projects/powerball/matlab/VAC/raw_data/clover_kang_indirect_var_adm_schunk.csv"  # TODO
num_reference_points = 600  # TODO number of reference points
center_x = 0.0  # TODO clover center x [m]
center_y = 0.0  # TODO clover center y [m]
petal_radius = 0.125  # TODO clover size [m]
band_width = 0.045  # TODO corridor thickness [m]
clover_rotation_deg = 0.0  # TODO clover rotation [deg]
...
def generate_four_leaf_clover_xy():
    """
    Generate the four-leaf clover centerline.
    ...
    theta = np.linspace(0.0, 2.0 * np.pi, num_reference_points,
                        endpoint=False)
    rot = np.deg2rad(clover_rotation_deg)

    radius = petal_radius * np.cos(2.0 * theta)

    x_ref = center_x + radius * np.cos(theta + rot)
    y_ref = center_y + radius * np.sin(theta + rot)

    return x_ref, y_ref


def generate_clover_boundaries(x_ref, y_ref):
    """
    Generate the left and right boundaries of the clover band.
...
def main():
    """
    Plot the clover reference, boundaries, and robot XY trajectory.
    """
    x_ref, y_ref = generate_four_leaf_clover_xy()
    x_left, y_left, x_right, y_right = \
        generate_clover_boundaries(x_ref, y_ref)
    x_robot, y_robot = read_robot_xy()
    x_robot, y_robot = transform_robot_xy(x_robot, y_robot)

    plt.figure("four_leaf_clover_and_robot_trajectory")
    plt.plot(x_ref, y_ref, color="tab:blue", linewidth=2.0,
             label="Four-leaf clover reference")
    plt.plot(x_left, y_left, color="tab:cyan", linewidth=1.2,
             linestyle="--", label="Clover left boundary")
    plt.plot(x_right, y_right, color="tab:green", linewidth=1.2,
             linestyle="--", label="Clover right boundary")
    plt.plot(x_robot, y_robot, color="tab:red", linewidth=1.5,
             label="Robot end-effector trajectory")
    plt.xlabel("X [m]")
    plt.ylabel("Y [m]")
    plt.title("Four-leaf clover reference and robot trajectory")
    plt.axis("equal")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
