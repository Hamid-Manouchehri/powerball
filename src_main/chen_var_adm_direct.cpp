/*
Direct variable admittance controller for Schunk Powerball pHRI.
...
Outputs:
    ...
    CSV log: force, velocity, damping, and joint data for analysis.

Controller:
    The planar force magnitude is interpreted as direct human
    intention. Small force means precise motion, so damping is high.
    Large force means fast motion, so damping is low. The Cartesian
    admittance velocity is converted to joint velocity with damped
...
// Set true to record Myo band EMG/IMU to a separate CSV.
static const bool USE_MYO = false;  // TODO
static const bool CONSTANT_DAMP = false; // TODO true: fixed damping
float const_damp = 100.0f;  // TODO range: 10 to 150 Ns/m
float cmin_pos   = 10.0f;   // TODO low damping, range: 10 to 100 Ns/m
float cmax_pos   = 100.0f;  // TODO high damping, range: 20 to 150 Ns/m
float M_inertia  = 8.0f;    // TODO virtual mass, range: 1 to 10 kg

float F_min = 1.0f;   // TODO low force threshold, range: 0.5 to 3 N
float F_max = 10.0f;  // TODO high force threshold, range: 5 to 15 N
...
Vector<6,float> Qdot_a = Zeros;  // Schunk joint velocity
...
)
{
    ...
    f << "    \"CONSTANT_DAMP\": " << (CONSTANT_DAMP ? "true" : "false") << ",\n";
    f << "    \"const_damp\": " << const_damp << ",\n";
    f << "    \"F_min\": " << F_min << ",\n";
    f << "    \"F_max\": " << F_max << ",\n";
    f << "    \"cmin_pos\": " << cmin_pos << ",\n";
    f << "    \"cmax_pos\": " << cmax_pos << ",\n";
    f << "    \"Md_diag\": ["
      << Md_diag[0] << "," << Md_diag[1] << "," << Md_diag[2] << ","
      << Md_diag[3] << "," << Md_diag[4] << "," << Md_diag[5]
      << "]\n";
    f << "  }\n";
    f << "}\n";
}
...
void Myo_log(std::atomic<bool>* stopFlag, std::string subject)
{
    ...
    std::string myo_path = "/home/srisadha/hamid_powerball/src_main/data/admittance/chen_var_adm_data/" + subject + "myo.csv";
...
static float compute_direct_damping(float force_magnitude)
{
    ...
    Larger force means the human wants faster motion, so damping
    decreases from cmax_pos to cmin_pos.
    */
...
static float rate_limit_damping(float desired_damping, float previous_damping)
{
...
static void update_direct_damping()
{
    /*
    Outputs:
        c_des: desired damping before rate limit [Ns/m].
        Cd_diag: applied virtual damping vector [Ns/m].

    Direct approach is isotropic in the plane: one force magnitude
    gives the same damping for x and y.
    */

    float force_magnitude = compute_planar_force_magnitude();
    float direct_damping = compute_direct_damping(force_magnitude);

    c_des[0] = direct_damping;
    c_des[1] = direct_damping;

    float c_pos_x = rate_limit_damping(c_des[0], c_pos_x_prev);
    float c_pos_y = rate_limit_damping(c_des[1], c_pos_y_prev);
    ...
    {
        Cd_diag = makeVector(1.0f, 1.0f, 1.0f,
                             1.0f, 1.0f, 1.0f) * const_damp;
    }
    else
    {
        Cd_diag[0] = c_pos_x;
...
static void integrate_planar_admittance()
{
    ...
    Inputs:
        ...
        Cd_diag: virtual damping [Ns/m].
        Md_diag: virtual mass [kg].
...
void computations()
{
    ...
    // Convert force from sensor frame into robot base frame.
    F_cmd = Rmat * (R_F_offset * F_modified);

    // Direct intention uses force magnitude, not velocity.
    // Larger force -> lower damping -> more compliant robot.
    v_meas = J * Qdot_a;
    
    v_meas_lpf = (1.0f - v_meas_lpf_alpha) * v_meas_lpf_prev + v_meas_lpf_alpha * v_meas;
    v_meas_lpf_prev = v_meas_lpf;
    
    update_direct_damping();
...
int main(int argc, char** argv)
{
    ...
    // Log file
    std::string filepath = "/home/srisadha/hamid_powerball/src_main/data/admittance/chen_var_adm_data/" + SubName + "_schunk.csv";
    ...
    {
        std::string jsonpath = "/home/srisadha/hamid_powerball/src_main/data/admittance/json/chen_var_adm/" + SubName + "_schunk.hyperparams.json";
    ...
    Qe = Data(0.0078f, -0.354f, 1.82f, 0.0f, 0.968f, 0.0f);  // further in x direction
