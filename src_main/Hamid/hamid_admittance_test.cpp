...
int main(int argc, char** argv)
{
    ...
    folderpath = "../data/hamid/exp_1/" + SubName + "_damp_" + std::to_string(Damp) + "_01.csv";
    
    dataFile.open(folderpath);
    dataFile << "TimeStamp, Q1, Q2, Q3, Q4, Q5, Q6, dQ1, dQ2, dQ3, dQ4, dQ5, dQ6, FT1, FT2, FT3, FT4, FT5, FT6, Vx, Vy, Vz, omega_x, omega_y, omega_z" << endl;
    // timestamp vars
    char buffer[10];
    struct timeval tv;
    time_t curtime;
    ...
    while((cnt<90/dt) && (!stop_flag))
    {
        ...
        dataFile<<Q[0]<<","<<Q[1]<<","<<Q[2]<<","<<Q[3]<<","<<Q[4]<<","<<Q[5]<<","
                     <<Qdot_a[0]<<","<<Qdot_a[1]<<","<<Qdot_a[2]<<","<<Qdot_a[3]<<","<<Qdot_a[4]<<","<<Qdot_a[5]<<","
                      ...
                      <<vel[0]<<","<<vel[1]<<","<<vel[2]<<","<<vel[3]<<","<<vel[4]<<","<<vel[5]<<endl;
