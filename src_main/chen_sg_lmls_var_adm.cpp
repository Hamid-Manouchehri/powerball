...
Controller:
    ...
    2. Indirect intention: recent motion is used to estimate the
       velocity-curvature power-law parameters K and beta using
...
int power_law_window = 50;  // TODO paper N, number of history samples
...
int lm_max_iterations = 8;  // TODO LM iterations per control cycle
float lm_initial_lambda = 1e-3f;  // TODO LM damping, larger is safer
float lm_lambda_up = 10.0f;  // TODO multiply lambda after failed step
float lm_lambda_down = 0.3f;  // TODO multiply lambda after good step
float lm_min_step_norm = 1e-5f;  // TODO stop when parameter step is small
float lm_min_cost_improvement = 1e-7f;  // TODO stop if cost barely changes
...
static float compute_lm_power_law_cost(
    const std::vector<float>& curvature_values,
    const std::vector<float>& speed_values,
    float logK,
    float beta)
{
    /*
    Compute mean squared speed error for:
        v = K * kappa^(-beta)

    Inputs:
        curvature_values: valid local curvature samples [1/m].
        speed_values: valid measured speed samples [m/s].
        logK: log of velocity gain factor.
        beta: power-law exponent.

    Outputs:
        cost: mean squared speed residual [(m/s)^2].
    */
    ...
    return sum_squared_error / (float)count;
}

static void estimate_power_law_lm_from_history()
{
    /*
    Fits the power law with Levenberg-Marquardt nonlinear least squares:
        v = K * kappa^(-beta)

    Residual minimized directly in speed space:
        residual = measured_speed - K * kappa^(-beta)

    Parameters optimized by LM:
        p = [log(K), beta]
    ...
    */
    ...
    float beta = clampf(beta_hat, beta_min, beta_max);
    float logK = 0.0f;
    ...
    else
    {
        ...
        for (int i = 0; i < count; i++)
        {
            sum_logK += std::log(speed_values[i]) +
                        beta * std::log(curvature_values[i]);
        }

        logK = sum_logK / (float)count;
    }

    logK = clampf(logK, lm_logK_min, lm_logK_max);
    ...
    for (int iteration = 0; iteration < lm_max_iterations; iteration++)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            float J0 = speed_fit;
            float J1 = -log_curvature * speed_fit;
        ...
        float A00 = H00 + lambda * (H00 + 1e-6f);
        float A01 = H01;
        float A11 = H11 + lambda * (H11 + 1e-6f);
        float determinant = A00 * A11 - A01 * A01;

        if (fabs(determinant) < 1e-9f)
            break;
        ...
        float candidate_beta =
            clampf(beta + step_beta, beta_min, beta_max);

        float candidate_cost = compute_lm_power_law_cost(
            curvature_values, speed_values, candidate_logK, candidate_beta);

        if (candidate_cost < cost)
        {
            float cost_improvement = cost - candidate_cost;
            logK = candidate_logK;
            beta = candidate_beta;
            cost = candidate_cost;
    ...
    for (int i = 0; i < count; i++)
        mean_speed += speed_values[i];
    mean_speed /= (float)count;
                                       ...
                                       const std::string& data_csv_path)
{
    ...
    f << "    \"power_law_estimator\": "
      << "\"levenberg_marquardt_nonlinear_ls\",\n";
    f << "    \"power_law_fit_space\": \"speed\",\n";
    f << "    \"power_law_window\": " << power_law_window << ",\n";
    f << "    \"power_law_min_points\": " << power_law_min_points << ",\n";
    f << "    \"lm_max_iterations\": " << lm_max_iterations << ",\n";
    f << "    \"lm_initial_lambda\": " << lm_initial_lambda << ",\n";
    f << "    \"lm_lambda_up\": " << lm_lambda_up << ",\n";
    f << "    \"lm_lambda_down\": " << lm_lambda_down << ",\n";
    f << "    \"lm_min_step_norm\": " << lm_min_step_norm << ",\n";
    f << "    \"lm_min_cost_improvement\": "
